package com.example.demo.collect;

import java.math.BigDecimal;
import java.util.*;
import java.util.stream.Collectors;

public class TestDemoStudentList {

    public static void main(String[] args) {
        List<Student> list = new ArrayList<>();
        Student student1 = new Student();
        student1.setAge("12");
        student1.setSex(0);
        Student student2 = new Student();
        student2.setAge("13");
        student2.setSex(2);
        Student student3 = new Student();
        student3.setAge("11");
        student3.setSex(1);
        Student student4 = new Student();
        student4.setAge("18");
        student4.setSex(1);
        Student student5 = new Student();
        student5.setAge("18");
        student5.setSex(0);
        Student student6 = new Student();
        student6.setAge("18");
        student6.setSex(2);
        Student student7 = new Student();
        student7.setAge("18");
        student7.setSex(2);
        list.add(student1);
        list.add(student2);
        list.add(student3);
        list.add(student4);
        list.add(student5);
        list.add(student6);
        list.add(student7);

        List<Demo> demos = new ArrayList<Demo>();
        System.out.println("原始数据 组装list<demo>*******************");
        //将list数据重新组装到一个demosList中 然后输出demosList
        demos = list.stream().map(student -> new Demo(student.getAge(), student.getSex())).collect(Collectors.toList());
        demos.forEach(demo -> {
            System.out.println("年龄 " + demo.getAge() + "  性别 " + demo.getSex() + ",");
        });

        //只提取sex 为0得数据
        System.out.println("只提取sex 为0得数据*******************");
        List<Demo> list1 = demos.stream().filter(demo -> demo.getSex() == 0).collect(Collectors.toList());
        list1.forEach(demo -> {
            System.out.println("年龄 " + demo.getAge() + "  性别 " + demo.getSex() + ",");
        });

        // 筛选年龄大于12岁
        System.out.println("筛选年龄大于12岁的*************");
        List<Demo> demoFilter = demos.stream().filter(demo -> Integer.valueOf(demo.getAge()) > 12).collect(Collectors.toList());
        demoFilter.forEach(demo -> {
            System.out.println("年龄 " + demo.getAge() + "  性别 " + demo.getSex() + ",");
        });

        // 按照年龄升序排序
        System.out.println("按照年龄升序排序******************");
        List<Demo> demoSort = demos.stream().sorted((s1, s2) -> s1.getAge().compareTo(s2.getAge())).collect(Collectors.toList());
        demoSort.forEach(demo -> {
            System.out.println("升序年龄 " + demo.getAge() + "  性别 " + demo.getSex() + ",");
        });

        // 按照年龄降序排序
        // 按照年龄升序排序
        System.out.println("按照年龄降序排序******************");
        List<Demo> demoSortDesc = demos.stream().sorted((s1, s2) -> s1.getAge().compareTo(s2.getAge())).collect(Collectors.toList());
        demoSortDesc.forEach(demo -> {
            System.out.println("降序年龄 " + demo.getAge() + "  性别 " + demo.getSex() + ",");
        });

        // 倒序
        System.out.println("倒序****************");
        ArrayList<Demo> demoArray = (ArrayList<Demo>) demos;

        Comparator<Demo> comparator = (h1, h2) -> h1.getAge().compareTo(h2.getAge());
        demos.sort(comparator.reversed());
        //or
        demos.sort(Comparator.comparing(Demo::getAge).reversed());

        demos.forEach(demo -> {
            System.out.println("年龄 " + demo.getAge() + "  性别 " + demo.getSex() + ",");
        });


        // 多条件正序
        System.out.println("多条件排序正序****************");
        demoArray.sort(Comparator.comparing(Demo::getSex).thenComparing(Demo::getAge));
        demoArray.forEach(demo -> {
            System.out.println("年龄 " + demo.getAge() + "  性别 " + demo.getSex() + ",");
        });


        // 多条件倒序https://www.cnblogs.com/invoker-/p/7709411.html
        System.out.println("多条件排序倒序 sex  倒序****************");
        demoArray.sort(Comparator.comparing(Demo::getSex).reversed().thenComparing(Demo::getAge));
        demoArray.forEach(demo -> {
            System.out.println("年龄 " + demo.getAge() + "  性别 " + demo.getSex() + ",");
        });

        // 按照年龄分组
        System.out.println("根据age分组结果为Map****************");
        Map<String, List<Demo>> demoOder = demos.stream().collect(Collectors.groupingBy(Demo::getAge));
        System.out.println(demoOder);
        for (Map.Entry<String, List<Demo>> entryUser : demoOder.entrySet()) {
            String key = entryUser.getKey();
            System.out.println(key);
            List<Demo> entryUserList = entryUser.getValue();
            entryUserList.forEach(demo -> {
                System.out.println("年龄 " + demo.getAge() + "  性别 " + demo.getSex() + ",");
            });

        }

        //stream.filter(Objects::nonNull)
        demos.stream().filter(x -> x!=null).forEach(demo -> {System.out.println(""+demo.getSex());
        });
        List<Student> studentList=new ArrayList<>();
        demos.stream().filter(x -> x!=null).filter(x -> x.getAge()!=null).forEach(
                demo -> {
                    Student student=new Student();
                    student.setAge("`100");
                    student.setSex(3);
                    studentList.add(student);
                    Student student10=new Student();
                    student10.setAge("`aa");
                    student10.setSex(400);
                    studentList.add(student10);
                }
        );
  /*      Date minEntryDate = studentList.stream().map(User::getEntryDate).min(Date::compareTo).get();

        //最大
        Date maxEntryDate = userList.stream().map(User::getEntryDate).max(Date::compareTo).get();*/
   //list 转换map
     /*   Map<Long, User> userMap = userList.stream().collect(Collectors.toMap(User::getId, a -> a,(k1,k2)->k1));*/
        List<Student> list2=studentList.stream().filter(stu -> stu.getSex()==400).collect(Collectors.toList());
        System.out.println(""+list2);

        //求和
        int sumSex = demos.stream().mapToInt(Demo::getSex).sum();

        System.out.println(sumSex);

//        //BigDecimal求和
//        BigDecimal totalQuantity = userList.stream().map(User::getFamilyMemberQuantity).reduce(BigDecimal.ZERO, BigDecimal::add);

    }


}
